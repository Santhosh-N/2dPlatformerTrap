...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        else if (state == State.Flee)
        {
            ...
            {
                rb.linearVelocity = new Vector2(assaultSpeed * dir, rb.linearVelocity.y);
            }
    ...
    Vector3 RandomTarget(bool turnBack = false, int direction = 0)
    {
        ...
        if (turnBack)
        {
            distance *= -Mathf.Sign(rb.linearVelocity.x);
        }
        else if (direction != 0)
        {
            distance *= direction;
    ...
    void OnDetectTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.tag == "Player" && !other.isTrigger)
        {
            ...
            if (other.GetComponent<Player>().state == Player.State.Dead) return;
    ...
    void OnHitTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.tag == "Player" && !other.isTrigger)
        {
            ...
            other.GetComponent<Player>().Damage(force);
        }
